...
import { MPlaceOpeningHoursInterface, PlaceControllerInterface, PlacePropsInterface } from 'spelieve-common/lib/Interfaces';
...
export const PPA002PlaceController = ({ place_id, language }: PlacePropsInterface): PlaceControllerInterface => {
    ...
    const displayOpeningHours = (openingHours: MPlaceOpeningHoursInterface[] | undefined): string | Array<[string, string]> => {
        if (!openingHours) {
        ...
        const days: { [key: number]: string } = { 0: 'Sunday', 1: 'Monday', 2: 'Tuesday', 3: 'Wednesday', 4: 'Thursday', 5: 'Friday', 6: 'Saturday' };
